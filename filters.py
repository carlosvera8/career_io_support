...
REMOTE_PATTERN = re.compile(
    r"\b(remote|anywhere|distributed|worldwide|work\s+from\s+home|wfh|global)\b",
    re.IGNORECASE,
)


def is_remote(job: dict) -> bool:
    """Return True if the job location or tags indicate remote work."""
    location = job.get("location_raw", "")
    tags = " ".join(job.get("tags", []))
    return bool(REMOTE_PATTERN.search(location) or REMOTE_PATTERN.search(tags))
...
def is_full_time(job: dict) -> bool:
    ...
    # Only exclude if it's strongly contract-signaling (not just mentioning contractors)
...
def meets_salary_threshold(
    job: dict, threshold: int = 200_000
) -> tuple[bool | None, str]:
    ...
    if s_min is not None or s_max is not None:
        ...
        return best >= threshold, salary_str
...
def apply_all_filters(job: dict, company_names: set[str]) -> bool:
    ...
    so Levels.fyi lookups can be applied as a fallback.
    """
    # Must be from a company on our curated list
